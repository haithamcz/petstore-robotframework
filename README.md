...
Tests are the start will clear out the environment from any previous test data to ensure test consistency, please be careful running them against your internal environment.
### Running the tests
To run the tests in terminal navigate to api or web test cases folder (petstore/testcases/api)and type: robot api_tests.robot 
The test will run and a test report will be generated in petstore/testcases/api (you can generate test report to different directory by adding -d to the run command)
